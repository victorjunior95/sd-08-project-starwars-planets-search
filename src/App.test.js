...
xdescribe('1 - Faça uma requisição para o endpoint `/planets` da API de Star Wars e preencha uma tabela com os dados retornados, com exceção dos da coluna `residents`', () => {
  beforeAll(mockFetch);
  beforeEach(cleanup);
...
xdescribe('4 - Não utilize filtros repetidos', () => {
  beforeAll(mockFetch);
  beforeEach(cleanup);
...
xdescribe('5 - Apague o filtro de valores numéricos e desfaça as filtragens dos dados da tabela ao clicar no ícone de `X` de um dos filtros', () => {
  beforeAll(mockFetch);
  beforeEach(cleanup);
...
xdescribe('6 - Ordene as colunas de forma ascendente ou descendente', () => {
  beforeAll(mockFetch);
  beforeEach(cleanup);
